...
engine = create_engine("sqlite:///PriceList "+str(date)+".db")
client = Client()
info = client.get_exchange_info()
symbols = [x['symbol'] for x in info['symbols']]
exclude = ['UP', 'DOWN', 'BEAR', 'BULL']
non_lev = [symbol for symbol in symbols if all(excludes not in symbol for excludes in exclude)]
relevent = [symbol for symbol in non_lev if symbol.endswith('USDT')]
multi = [i.lower() + '@trade' for i in relevent]
...
if __name__ == "__main__":
    ...
    while(count < num_retry):
        try:
            ...
            count += 1
            logfile.write("Error Occurred retrying\n"+Exception)
        finally:
            if(count>=num_retry):
                logfile.close()
